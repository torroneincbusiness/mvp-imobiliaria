...
# 1. Configuração
# Certifique-se de configurar a GOOGLE_API_KEY no seu ambiente
genai.configure(api_key=os.getenv("GOOGLE_API_KEY"))
# Corrigido: gemini-3.5-flash não existe, use 1.5-flash ou 2.0-flash
...
# 4. Função: Qualificação (Motor de Score)
def processar_lead(texto_usuario):
    ...
    # CORREÇÃO: Dicionário fechado corretamente
    return {
        "score": score,
        "temperatura": temperatura,
        "dados_extraidos": dados,
        "dossie": f"Motivo: {dados['motivo_mudanca']} | Sugestões Automáticas: {', '.join(sugestoes) if sugestoes else 'Nenhuma'}"
    }

# 5. Função: Recomendação (Motor Concierge)
